...
export default function Addresses() {
    ...
    const fetchAddresses = async (page, size, search = "") => {
        setLoading(true);
        try {
            const skip = (page - 1) * size;
            const res = await fetch(`http://localhost:8000/api/v1/addresses/?skip=${skip}&limit=${size}`);
            ...
            if (search) {
                ...
                items = items.filter(a =>
                    (a.first_name && a.first_name.toLowerCase().includes(lowerSearch)) ||
                    (a.last_name && a.last_name.toLowerCase().includes(lowerSearch)) ||
                    (a.city && a.city.toLowerCase().includes(lowerSearch))
                );
            }
    ...
    useEffect(() => {
        fetchAddresses(currentPage, perPage, searchTerm);
        ...
        },
        {
            ...
            cell: row => (
                <div className="flex gap-2">
                    <button
                        onClick={() => navigate(`/admin/addresses/${row.id}`)}
    ...
    return (
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
            <div className="sm:flex sm:items-center sm:justify-between mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Addresses</h1>
                    <p className="mt-2 text-sm text-gray-700">
                        A list of all physical locations mapped to users and merchants.
                    </p>
                </div>
            ...
            <div className="bg-white px-4 py-5 sm:px-6 rounded-none outline-1 -outline-offset-1 outline-gray-200 shadow-sm mb-6 flex flex-col sm:flex-row gap-4 items-center justify-between">
                <div className="relative w-full sm:max-w-xs">
                    ...
                    <input
                        type="search"
                        placeholder="Search addresses (name, city)..."
